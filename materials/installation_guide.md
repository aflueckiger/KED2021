...
Please install the following Python packages  with the Anaconda Package Manager conda.
...
Using the same Anaconda Prompt as before, you can install the following Python packages with the Anaconda Package Manager conda.
...
9. Create symbolic links to have easy access to local files from your in the Ubuntu Bash. For example, you may want to mount (i.e., make accessible) the directory `Downloads` from your Windows  environment in your Home directory on Ubuntu:

   ```bash
    cd ~
    ln -s /mnt/c/Users/YOUR_WINDOWS_USERNAME/Downloads
   ```
